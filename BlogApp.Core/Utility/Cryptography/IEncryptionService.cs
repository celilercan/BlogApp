...
namespace BlogApp.Core.Utility.Cryptography
{
    public interface IEncryptionService
    {
        ...
        /// <param name="encryptionPrivateKey">Encryption private key</param>
        /// <param name="isUrl">If value comes from url parameter set it true</param>
        /// <returns>Decrypted text</returns>
        string DecryptText(string cipherText, string encryptionPrivateKey = "", bool isUrl = false);

        string EncyrptByHex(string textValue);
        string DecryptByHex(string hexValue);
        string EncryptPassword(string password);
        string DescryptPassword(string decryptedPassword, bool isUrl = false);
    }
}
